...
set -eu
for year in $(seq 2009 1 2023) ; do
ssh panagopkonst@memos1.troias.offices.aueb.gr "cd ../../; ls -l /taq93-23/taq_msec${year}/m*/mastm_*" | awk '{print $9, $5}' | awk -F/ '{print $5, $6}' | sed 's/.gz//' | sed 's/.bz2//' |sort -n > memo_mastm_size
ssh wrds ls -l /wrdslin/nyse/taq_msec${year}/m*/mastm_*.sas7bdat |awk '{print $9, $5}' |awk  -F/ '{print $6,$7}'|sort -n >wrds_mastm_size
join wrds_mastm_size memo_mastm_size | awk '{print $0, $3 / $2 * 100}' >compression_rates
